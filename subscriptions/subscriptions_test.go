...
func TestCreateSubscription_HTTPOK(t *testing.T) {
	...
	subscriptionsDB.EXPECT().Put("http,GET,%2F", []byte(`{"subscriptionId":"http,GET,%2F","eventType":"http","functionId":"func","method":"GET","path":"/"}`), nil).Return(nil)
	...
	_, err := subs.CreateSubscription(&Subscription{ID: "testid", EventType: "http", FunctionID: "func", Method: "GET", Path: "/"})
...
func TestCreateSubscription_HTTPValidationError(t *testing.T) {
	...
	_, err := subs.CreateSubscription(&Subscription{ID: "testid", EventType: "http", FunctionID: "func"})
...
func TestCreateSubscription_OK(t *testing.T) {
	...
	subscriptionsDB.EXPECT().Put("test,func,%2F", []byte(`{"subscriptionId":"test,func,%2F","eventType":"test","functionId":"func","path":"/"}`), nil).Return(nil)
	functionsDB := mock.NewMockStore(ctrl)
	functionsDB.EXPECT().Exists("func", &store.ReadOptions{Consistent: true}).Return(true, nil)
	subs := &Subscriptions{SubscriptionsDB: subscriptionsDB, FunctionsDB: functionsDB, Log: zap.NewNop()}

	_, err := subs.CreateSubscription(&Subscription{ID: "testid", EventType: "test", FunctionID: "func"})
...
func TestCreateSubscription_ValidationError(t *testing.T) {
	...
	assert.Equal(t, err, &ErrSubscriptionValidation{original: "Key: 'Subscription.EventType' Error:Field validation for 'EventType' failed on the 'required' tag\nKey: 'Subscription.FunctionID' Error:Field validation for 'FunctionID' failed on the 'required' tag"})
}
...
func TestCreateSubscription_EndpointPathConflictError(t *testing.T) {
	...
	_, err := subs.CreateSubscription(&Subscription{ID: "testid", EventType: "http", FunctionID: "func", Method: "GET", Path: "/:id"})
...
func TestCreateSubscription_PutError(t *testing.T) {
	...
	subscriptionsDB := mock.NewMockStore(ctrl)
	subscriptionsDB.EXPECT().Get("http,GET,%2F", gomock.Any()).Return(nil, errors.New("KV sub not found"))
	subscriptionsDB.EXPECT().Put("http,GET,%2F", []byte(`{"subscriptionId":"http,GET,%2F","eventType":"http","functionId":"func","method":"GET","path":"/"}`), nil).Return(errors.New("KV Put err"))
	endpointsDB := mock.NewMockStore(ctrl)
	endpointsDB.EXPECT().List("", gomock.Any()).Return([]*store.KVPair{}, nil)
	endpointsDB.EXPECT().Put("GET,%2F", []byte(`{"endpointId":"GET,%2F"}`), nil).Return(nil)
	functionsDB := mock.NewMockStore(ctrl)
...
func TestDeleteSubscription_DeleteEndpointOK(t *testing.T) {
	...
	kv := &store.KVPair{Value: []byte(`{"subscriptionId":"testid","eventType":"http","functionId":"f1","method":"GET","path":"/"}`)}
	subscriptionsDB := mock.NewMockStore(ctrl)
	subscriptionsDB.EXPECT().Get("testid", gomock.Any()).Return(kv, nil)
	subscriptionsDB.EXPECT().Delete("testid").Return(nil)
	endpointsDB := mock.NewMockStore(ctrl)
...
func TestGetAllSubscriptions_OK(t *testing.T) {
	...
	kvs := []*store.KVPair{
		{Value: []byte(`{"subscriptionId":"s1","eventType":"test","functionId":"f1"}`)},
		{Value: []byte(`{"subscriptionId":"s2","eventType":"test","functionId":"f2"}`)},
	}
	subscriptionsDB := mock.NewMockStore(ctrl)
	...
	assert.Equal(t, []*Subscription{
		{ID: SubscriptionID("s1"), EventType: "test", FunctionID: functions.FunctionID("f1")},
		{ID: SubscriptionID("s2"), EventType: "test", FunctionID: functions.FunctionID("f2")},
	}, list)
}
